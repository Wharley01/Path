...
class Database{
     ...
     * @param $columns
     */
    public function Query($query,$type = 'fetch'){
//        echo $query;
//        var_dump($this->query_data['bind_data']);
        $exe = $this->db_con->prepare($query);
        if(@$this->query_data['bind_data']){
    ...
    public function bind_data(Array $data){
        $this->query_data["bind_data"] = $data;
    }
...
//        echo $query.'<br><br>';
        try{
            $exe = $this->db_con->prepare($query);
            $exe->execute(@$this->query_data['bind_data']);
            $this->last_id = $this->db_con->lastInsertId();
    ...
    public function Get($is_array = true){
        $query = "";
        if(!$this->query_data['table'])
            throw new \Exception("Specify Table name");
        if(!$this->query_data['column'])
            throw new \Exception("Specify Column");
